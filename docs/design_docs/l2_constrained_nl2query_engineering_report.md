...
> **状态（2026-09-04）：未实施的设计方案。** `agent/nodes/structured_query/` 目录、QueryPlan、能力注册中心均**不存在于代码中**，本方案尚未立项。本文"现状"章节已对齐当前代码；请勿将本文件当作现状文档或实现说明阅读。
>
...
保留现有 MySQL FULLTEXT 混合召回思路（FULLTEXT → LIKE → 全表兜底；原 Milvus 结构化语义召回臂已于 2026-09-04 全面退役，若恢复语义召回须另立新决策），但由 Plan Builder 统一组装：
...
QueryPlan
  ...
  → Candidate Generation：FULLTEXT OR / LIKE 兜底
...
| Keyword | 企业简称、主营业务词、项目主题词 | FULLTEXT OR + LIKE 兜底 |
...
- temperature 设为 0；
- 模型身份一律走 .env 现有 `LLM_MODEL` / `LLM_API_KEY` / `LLM_BASE_URL`（项目 D4/D6 约束：模型配置 .env 单一事实源，禁止代码内写死模型名）；若当前模型 structured output 不稳定，改用 Tool Calling + Pydantic schema 描述，再二次解析。
...
Planner 复用现有 `LLM_*` 模型身份配置，不新增模型提供方开关。关闭后节点立即回到当前 SearchIntent 链路。
