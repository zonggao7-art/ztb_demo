...
> **状态（2026-09-04）：未实施的规划文档。** `quality_guard` / Critic 节点未落地（graph.py 中无此节点），本文规划将在后续阶段实施；阅读时请勿将文中架构当作现状。
> **基线说明**：文中 `agent_architecture.md` 为早期骨架稿（已删除），现行架构以 [project_overview.md](../analysis_docs/project_overview.md) 为准；文中 "DeepSeek" 等模型名指代撰写时的对话模型，实际模型一律由 `.env LLM_MODEL` 声明。
>
> 前置依赖：
...
1. **成本爆炸**。每次问答都会触发全部 4 个业务节点，与现有"精准单分支"路由哲学（路由枚举 1:1 精准路由，见 `agent/router.py`）相悖。
2. **逻辑混乱**。用户在闲聊时被触发 price_inquiry 查询，违反 `general_chat` 节点"纯 LLM 闲聊、无知识库"的设计原则。
3. **增加不可控的副作用**。`ThreadPoolExecutor` 的并行查询变成强制并行，每个业务节点都会走 MySQL/Milvus，与现有架构"按需访问基础设施"的工程原则冲突。
...
## 3. 新架构图（基于现行 StateGraph 骨架扩展，见 project_overview.md）
...
现有架构的五项工程优化原则（原载《组件工作机制.md》，该文件已不在仓库）是现有架构的工程基石。质量把关层的引入需要保持与这五项原则的一致性：
