...
> **状态（2026-09-04）：未实施的规划文档。** 多模态（图像识别/OCR）能力未落地，本文规划将在后续阶段实施；阅读时请勿将文中架构当作现状。
> **基线说明**：文中 `agent_architecture.md` 为早期骨架稿（已删除），现行架构以 [project_overview.md](../analysis_docs/project_overview.md) 为准。
>
> 前置依赖：
> - [three_core_modules_design_and_feasibility.md](./three_core_modules_design_and_feasibility.md)（三核精细化设计）
> - [deep_agents_integration_design.md](./deep_agents_integration_design.md)（质量把关 Critic 模式，同为未实施规划）
...
- **离线/批处理场景**：继续以 `MinerUParser` 为主（本地 magic-pdf CLI），对 `raw_pdfs/` 中的历史法规文档完成结构化提取，复用现有 `SemanticChunker → Embedding → Milvus` 全流程；
...
**关键结论**：MinerU 当前定位是"**离线 PDF → Markdown 结构化转换器**"，其输出（Markdown 文本）已通过 `TextCleaner → SemanticChunker → Embedding → Milvus` 全链路索引到 `public_kb` 集合（详见 [project_overview.md §3.3](../analysis_docs/project_overview.md) L289-308）。
...
| **UC-MM-05** | 历史扫描件存量入库 | `public_kb` + `MinerU` | **方案 A**（MinerU 扩展） | 批量处理历史扫描件 PDF | 🟡 P1 |
...
**触发路由**：在 `RouterIntent` 中，`price_inquiry` 已支持"上传图片分析"作为子场景（详见 [router_penalty_check_fix_report.md §6.4](../analysis_docs/router_penalty_check_fix_report.md) L358 "多模态查询 | 支持上传图片/表格进行查询"）。
...
参考 [citation_schema.md](./citation_schema.md)，多模态场景下的 Citation 必须包含：
...
R1-R7 规则（[citation_schema.md §3](./citation_schema.md)）全部继续生效，多模态场景下：
...
参考 [deep_agents_integration_design.md](./deep_agents_integration_design.md) 中规划的 `quality_guard` Critic 节点，多模态场景的输出同样需要三档决策：
...
参考现有架构的五项核心工程优化原则（原载《组件工作机制.md》，该文件已不在仓库）：
...
| 质量门控扩展 | `deep_agents_integration_design.md` | §2.4 R1-R7 规则扩展 R8-R9 |
