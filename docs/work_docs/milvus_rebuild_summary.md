...
本次重建以 [milvus_retrieval_analysis_report.md](../analysis_docs/milvus_retrieval_analysis_report.md) 中识别的五项隐患为驱动，通过以下四个核心升级改造检索全链路：
...
## 8. 遗留项关闭登记（2026-09-03，数据重建阶段完成）

> 本节为 2026-09-03 数据重建（阶段二）完成后的统一关闭登记，上文 §5/§7 的历史状态以本节为准。
> 详见 `work_docs/知识库Milvus重建与链路验证工作总结报告_20260903.md` 与 `design_docs/知识库Milvus重建与混合检索链路验证设计方案_20260903.md`。

| # | 功能点 | 关闭状态 | 证据 |
|---|--------|:---:|------|
| 1 | BM25 Function 实际启用 | ✅ **关闭** | Milvus 2.6.23 上 `text_bm25` Function 真实持久化（建库 describe 自检 + T2 全链路日志；2.4 时代的阻塞点已随阶段一升级消除） |
| 2 | SPARSE_INVERTED_INDEX 在线运行 | ✅ **关闭** | `sparse_vector` 字段 + BM25 度量稀疏索引在位（T1），稀疏路精确命中验证通过（T3：条款号直查逐字命中） |
| 3 | 混合检索端到端验证 | ✅ **关闭** | dense(30)+sparse(30)→RRF(k=60)→Reranker→动态阈值 全链路日志验证通过（T2）；语义/条款号/拒答三类 query 冒烟通过（T4/T3b/T7） |
| 6 | 检索精度量化对比 | 🔄 **口径更新** | 由"v1.5 vs bge-m3 对比"改为 106 题引用评测建基线（`testset_knowledge.jsonl` 缺失，评测后补，见设计方案 Q4） |
| 4 | `milvus/backup.sh` 备份脚本 | ⬜ 保留 | 仍属阶段二后续任务（本轮以 Milvus 卷 tar 备份 + 产物可再生为回滚保障） |
| 5 | Reranker 备选本地方案 | ⬜ 保留 | 维持原状 |

**新数据基线**：`public_kb` 集合 = 3 本书 920 块（chunk_uid 主键，digest `c47d30c9…`），Reranker 分数分布：18 条领域 query 全部 ≥0.93（阈值 0.40 放行）、2 条无关 query <0.06（阈值 0.50 拒答），双峰分离清晰，三级动态阈值初值判定合理。
