...
"""
CSV → MySQL 批量导入脚本
将 raw_tables/ 目录下的 CSV 文件导入 ztb_clean 数据库。
...
默认仅导入当前在用的 3 张业务表（company_info / company_penalty /
bid_project）。product_info 已随产品查询功能线下线（2026-08-10），
如需导入须显式指定且库中须已建表。
...
# 默认导入清单：当前在用的 3 张表（product_info 已下线，见 Q1 决策）
DEFAULT_TABLES = ["company_info", "company_penalty", "bid_project"]
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"逗号分隔的表清单（默认 {','.join(DEFAULT_TABLES)}；"
             f"product_info 已下线，如需导入须显式指定且库中已建表）",
    )
    ...
    unknown = [t for t in tables if t not in TABLE_MAP.values()]
    if unknown:
        logger.error("Unknown tables: %s (valid: %s)", unknown, list(TABLE_MAP.values()))
        sys.exit(1)

    # 连接 MySQL
    conn = pymysql.connect(database=_CLEAN_DB, **_MYSQL_CONFIG)
    logger.info("Connected to MySQL %s:%s, database: %s",
                _MYSQL_CONFIG["host"], _MYSQL_CONFIG["port"], _CLEAN_DB)

    # 按固定顺序导入（主表在前）
    ordered = [t for t in ["company_info", "company_penalty", "product_info", "bid_project"]
               if t in tables]

    totals = {}
    for table_name in ordered:
        csv_file = next(name for name, tbl in TABLE_MAP.items() if tbl == table_name)
    ...
    for table_name, imported in totals.items():
        ...
        marker = "OK" if imported == db_count else "MERGED (dup keys)"
        print(f"  {table_name:<20s}: imported={imported:>6d},  DB rows={db_count:>6d}  [{marker}]")
